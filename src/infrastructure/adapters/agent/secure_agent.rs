...
/// Shell-backed environment, Anthropic [`ClaudeLlm`], fixed display name **SecureAgent**. The API key is resolved only in [`SecureAgent::new`] via a [`Runtime`] (see [`Runtime::get_secret`]); the core [`Agent`] does not carry a runtime.
