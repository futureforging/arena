...
use super::MessageLogFn;

// -----------------------------------------------------------------------------
// Peer protocol (line-based TCP messaging)
// -----------------------------------------------------------------------------
...
async fn write_sentinel(writer: &mut (impl AsyncWriteExt + Unpin)) -> io::Result<()> {
...
fn is_end_sentinel(line: &str) -> bool {
...
impl Tool for SendToPeerTool {
    ...
    async fn execute(&self, input: serde_json::Value, _context: &ExecutionContext) -> ToolResult {
        let message = match input.get("message").and_then(|v| v.as_str()) {
            Some(m) => m.to_string(),
            None => return ToolResult::error("Error: missing 'message' parameter"),
        };
