...
The workspace uses a **hexagonal** (ports-and-adapters) shape: **`verity-core`** is the **inner hexagon**—domain types and **ports** only (`Tool`, `ToolRegistry`, `Llm`, `Environment`, `Game`, `Arena`), with no Omnia or WASI dependencies. **`verity-tools`** holds pluggable tool implementations (`SecretsTool`, `HttpClientTool`, `ArenaClientTool`) built on **`verity-core`**’s `Tool` trait — including an in-process tool registration of the arena (the `"arena"` tool) so the agent's outbound capability is explicit and auditable. **`verity-adapters`** holds concrete implementations of `verity-core` ports against host environments and the protocol-shaped transport ports they compose over: today `CliEnvironment` (stdio), `DemoArena` (the Scaling Trust Arena over an injected `HttpTransport`), and the `transport::HttpTransport` port itself. **`secure-agent`** is the **application**: it assembles a tool registry, hosts the WASI-tied adapters (`WasiHttpAdapter` implementing `HttpTransport`, `WasiLlm`), and is built as the WASM guest. **`verity-runtime`** is **infrastructure** on the host: it loads the guest `.wasm` and wires Omnia/WASI **host adapters** (vault, HTTP, keyvalue, telemetry); it intentionally does **not** depend on `verity-core`, `verity-tools`, `verity-adapters`, or `secure-agent`.

| Directory | Crate | Role |
| --- | --- | --- |
| `core/` | `verity-core` | Shared domain types, trait ports (`Tool`, `ToolRegistry`, `Llm`, `Environment`, `Game`, `Arena`), game logic, and tests. |
...
| `adapters/` | `verity-adapters` | Concrete adapters that implement `verity-core` ports against host environments: `CliEnvironment` (stdio), `DemoArena` (Scaling Trust Arena over an injected `HttpTransport`), plus the `transport::HttpTransport` port that adapters compose over. Depends only on `verity-core`. |
| `secure-agent/` | `secure-agent` | **WASI guest** (`wasm32-wasip2` cdylib): example agent that assembles a tool registry and plays arena games inside the sandbox. Hosts the WASI-specific adapters that depend on Omnia guest crates: `WasiHttpAdapter` (implements `HttpTransport` against `omnia_wasi_http`) and `WasiLlm`. |
